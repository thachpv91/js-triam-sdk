...
`
      ...
      TriamSdk.StellarTomlResolver.resolve('acme.com')
        .then(stellarToml => {
...
`
      ...
      TriamSdk.StellarTomlResolver.resolve('acme.com', {allowHttp: true})
        .then(stellarToml => {
...
`
      ...
      TriamSdk.StellarTomlResolver.resolve('acme.com').should.be.rejectedWith(/Parsing error on line/).and.notify(done);
    });
    ...
    it("rejects when there was a connection error", function (done) {
      ...
      TriamSdk.StellarTomlResolver.resolve('acme.com').should.be.rejected.and.notify(done);
    });
    ...
    it("fails when response exceeds the limit", function (done) {
      ...
      var response = Array(TriamSdk.STELLAR_TOML_MAX_SIZE+10).join('a');
      let tempServer = http.createServer((req, res) => {
        res.setHeader('Content-Type', 'text/x-toml; charset=UTF-8');
        res.end(response);
      }).listen(4444, () => {
        TriamSdk.StellarTomlResolver.resolve("localhost:4444", {allowHttp: true})
