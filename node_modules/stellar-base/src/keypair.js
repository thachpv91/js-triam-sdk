...
export class Keypair {
  ...
  /**
   * Creates a new `Keypair` object from contract key.
   * @param {string} contractKey contract key (ex. `CB3KJPLFUYN5VL6R3GU3EGCGVCKFDSD7BEDX42HWG5BWFKB3KQGJJRMA`)
   * @returns {Keypair}
   */
  static fromContractKey(contractKey) {
    contractKey = StrKey.decodeContractKey(contractKey);
    if (contractKey.length !== 32) {
      throw new Error('Invalid Stellar contract key');
    }
    return new this({type: 'contract', contractKey});
  }
  ...
  // thuannd start
  xdrContractId() {
    return new xdr.ContractId.publicKeyTypeContract(this._publicKey);
  }
  // thuannd end
